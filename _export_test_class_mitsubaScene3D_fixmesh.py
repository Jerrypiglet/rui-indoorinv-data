'''
flip face normals from sampled poses
'''
import sys
import trimesh
# host = 'mm1'
...
scene_obj = mitsubaScene3D(
    ...
    cam_params_dict={
        ...
        'sample_pose_num': 800 if 'train' in opt.split else 20, # Number of poses to sample; set to -1 if not sampling
        'sample_pose_if_vis_plt': False, # images/demo_sample_pose.png, images/demo_sample_pose_bathroom.png
        'sample_pose_if_dump': False,
    }, 
...
'''
reload mesh to be fixed
'''
scene_obj.scene_params_dict.update({
    'shape_file': shape_file, 
})
scene_obj.load_mi_scene(scene_obj.mi_params_dict)
scene_obj.load_shapes(scene_obj.shape_params_dict, force=True)
del scene_obj.mi_rays_ret_list
scene_obj.process_mi_scene(scene_obj.mi_params_dict, if_postprocess_mi_frames=True, force=True)
# scene_obj.mi_sample_rays_pts(scene_obj.cam_rays_list)
assert hasattr(scene_obj, 'mi_rays_ret_list')
scene_obj.pts_from['mi'] = True
...
'''
Evaluator for scene: TO FLIP NORMALS
'''
if opt.eval_scene:
    if 'scene' not in shape_file:
        ...
        for k, v in _.items():
            if k in eval_return_dict:
                ...
                eval_return_dict[k] = _[k]
        if 'face_normals_flipped_mask' in eval_return_dict:
            ...
            if np.sum(face_normals_flipped_mask) > 0:
                ...
                shape_trimesh = trimesh.load_mesh(shape_file)
                _vertices, _faces =shape_trimesh.vertices, shape_trimesh.faces
                _faces[face_normals_flipped_mask] = _faces[face_normals_flipped_mask][:, [0, 2, 1]]
                trimesh.Trimesh(vertices=_vertices, faces=_faces).export(str(shape_file).replace('.obj', '_flipped.obj'))
                print(white_magenta('[FLIPPED] %d/%d inward face normals'%(np.sum(face_normals_flipped_mask), scene_obj.faces_list[0].shape[0])))