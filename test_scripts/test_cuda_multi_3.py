...
def run_one_gpu(i, opt, split, result_queue):
    ...
    # process_result_list = []
    # cmd = 'python tools/Mask3D/dump_openrooms_one_gpu.py --gpu_id {} --gpu_total {} --split {}'.format(i, opt.gpu_total, split)
    # _results = run_cmd(cmd)
    # print(_results)
    time.sleep(5)
    print("pid={} count={} SLEEP DONE".format(i, torch.cuda.device_count()))
