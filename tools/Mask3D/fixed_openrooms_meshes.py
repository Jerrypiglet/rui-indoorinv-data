...
all_shape_paths = Path('/newfoundland2/ruizhu/siggraphasia20dataset/uv_mapped_ori/').rglob('*.obj')
...
for shape_path in tqdm(list(all_shape_paths)):
    ...
    shape_path_dest = str(shape_path).replace('uv_mapped_ori', 'uv_mapped')
    Path(shape_path_dest).parent.mkdir(parents=True, exist_ok=True)    
    shape_tri_mesh.export(shape_path_dest)
