...
OFFSET = len(frame_id_list_dict['train']) if 'train' in frame_id_list_dict else 0

# for split in ['train', 'val']:
for split in ['val', '']:
   if split not in frame_id_list_dict:
      continue
   for frame_idx, frame_id in enumerate(frame_id_list_dict[split]):
      ...
      # assert fvp_relight is not None, str(relight_vis_path)
      if fvp_relight is None: continue
