...
def render_with_sg(lgtSGs, specular_reflectance, roughness, diffuse_albedo, normal, viewdirs, blending_weights=None, diffuse_rgb=None):
    '''
    :param lgtSGs: [M, 7], M=128 | => [..., M, 7], M=12
    :param specular_reflectance: [K=1, 3] | => 0.05 * [1, 3]
    :param roughness: [K=1, 1]; values must be positive | => [..., 1]
    :param diffuse_albedo: [..., 3]; values must lie in [0,1]
    :param normal: [..., 3]; ----> camera; must have unit norm
    :param viewdirs: [..., 3]; ----> camera; must have unit norm; negative of ray directions
    :param blending_weights: [..., K=1]; values must be positive, and sum to one along last dimension
    :return [..., 3]
    '''
    M = lgtSGs.shape[1]
    ...
    normal = normal.unsqueeze(-2).unsqueeze(-2).expand(dots_shape + [M, K, 3])  # [..., M, K, 3] | => [..., M, K=1, 3]

    # roughness = roughness + TINY_NUMBER
    # roughness = roughness + 0.1

    # viewdirs = viewdirs / (torch.norm(viewdirs, dim=-1, keepdim=True) + TINY_NUMBER)  # [..., 3]; ---> camera
    viewdirs = viewdirs.unsqueeze(-2).unsqueeze(-2).expand(dots_shape + [M, K, 3])  # [..., M, K, 3] | => [..., M, K=1, 3]
    ...
    inv_roughness_pow4 = 1. / (roughness * roughness * roughness * roughness + TINY_NUMBER)  # [..., K=1]
    # print(inv_roughness_pow4.shape)
    brdfSGLambdas = (2. * inv_roughness_pow4).unsqueeze(-2).unsqueeze(-2).expand(dots_shape + [M, K, 1])  # [..., M, K, 1]; can be huge
    brdfSGMus = (inv_roughness_pow4 / np.pi).unsqueeze(-2).unsqueeze(-1).expand(dots_shape + [M, K, 3]) # [..., M, K, 3]
