...
def parse_XML_for_shapes_global(root, root_uv_mapped, root_layoutMesh, scene_xml_root, root_EnvDataset, if_return_emitters=False, light_dat_lists=None, random_seed=0, scene_name_frame_id=None, if_not_write_representation_specific_info=False, main_xml_file=''):
    '''
    ...
    if if_return_emitters:
        ...
        emitter_dict['emitter_prop']['envMapPath'] = emitter_env.findall('string')[0].get('value')
        emitter_dict['emitter_prop']['envMapPath'] = str(Path(root_EnvDataset) / Path(emitter_dict['emitter_prop']['envMapPath']).name)
    ...
    for shape in shapes:
        ...
        if 'window' in shape_dict['filename']:
            if if_bright_outside:  # window that has light shining through
                ...
                shape_dict['emitter_prop']['if_env'] = True
                ...
                shape_dict['emitter_prop']['if_lit_up'] = max_envmap_scale > 1e-3
                ...
                if light_dat_lists['windows']:
                    ...
                    assert max_envmap_scale == light_dat['envScale']
                    ...
                    if not if_not_write_representation_specific_info:
                        ...
                        shape_dict['emitter_prop']['envMapPath'] = str(Path(root_EnvDataset) / Path(shape_dict['emitter_prop']['envMapPath']).name)
                        for envmap_key in ['recHalfEnvName', 'imHalfEnvName', 'lightXmlFile']:
                            shape_dict['emitter_prop'][envmap_key] = str(Path(scene_xml_root) / Path(shape_dict['emitter_prop'][envmap_key]).name)
    ...
    uv_mapped_idx = [shape_idx for shape_idx in range(len(shapes_list)) if Path(shapes_list[shape_idx]['filename']).name.endswith('uv_mapped.obj')]
    container_idx = [shape_idx for shape_idx in range(len(shapes_list)) if Path(shapes_list[shape_idx]['filename']).name.endswith('container.obj')]
    assert len(uv_mapped_idx)==1 and len(container_idx)==1
    other_idx = [shape_idx for shape_idx in range(len(shapes_list)) if shape_idx not in uv_mapped_idx+container_idx]
    shapes_list = [shapes_list[_] for _ in uv_mapped_idx+container_idx+other_idx]
