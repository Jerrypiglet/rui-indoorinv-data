...
for scene, out_name in zip(scenes, out_names):
    ...
    camera_intrinsic = mitsuba_scene.K # (3, 4)
    ...
    #     poses.append(c2w)
    poses = mitsuba_scene.pose_list # [(3, 4)]
    ...
    for idx, pose in tqdm(enumerate(poses)):
        ...
        shutil.copy(mitsuba_scene.im_sdr_file_list[idx], target_image)

        target_image_hdr = os.path.join(out_path, "image/%06d.exr"%(idx))
        shutil.copy(mitsuba_scene.im_hdr_file_list[idx], target_image_hdr)
        ...
        mask = (np.ones((mitsuba_scene.H, mitsuba_scene.W, 3)) * 255.).astype(np.uint8) # [TODO] load masks from mi!
        ...
        new_depth = mitsuba_scene.mi_depth_list[idx].astype(np.float32) / 1000.
        plt.imsave(target_image, new_depth, cmap='viridis')
        np.save(target_image.replace(".png", ".npy"), new_depth)

        mi_normal_global = mitsuba_scene.mi_normal_global_list[idx].astype(np.float32)
        R = pose[:3, :3]
        mi_normal_cam = (R.T @ mi_normal_global.reshape(-1, 3).T).T.reshape(mitsuba_scene.H, mitsuba_scene.W, 3) # images/demo_normal_gt_scannetScene_opencv.png
